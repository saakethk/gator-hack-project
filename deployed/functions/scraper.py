""" Scrape and filter relevant data sources to get topics """
...
import time
import json
from general import NAVIGATOR_CLIENT, REDDIT_CLIENT
from database import find_topic_by_name, get_info_about_topic, update_topic
...
def parse_website_topic(url: str, name: str):
    ...
    try:
        ...
        return True, summary_parsed["name"], summary_parsed["summary"], summary_parsed["pros"], summary_parsed["cons"]
    except Exception as error:
        return False, error, error, error, error
...
def get_reddit_posts(subreddit_name: str, limit: int, exceptions: list[str]) -> list[Topic]:
    ...
    for submission in subreddit.rising(limit=limit):
        ...
        if valid:
            ...
            in_database, id = find_topic_by_name(filtered_name) # type: ignore
            if in_database == False:
                ...
                # Get current internal_relevance_score
                score_key = "internal_relevance_score"
                curr_data = get_info_about_topic(id, score_key)[0][score_key] # type: ignore
                # Update the internal_relevance_score
                update_topic(id=id, column=score_key, value=curr_data+1) # type: ignore
    return topics
